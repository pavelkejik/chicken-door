...
private:
    ...
    const DoorEvents_t event;

public:
    Error(uint8_t debup, uint8_t debdown, uint8_t blck, uint8_t clear, const char *err_txt, ErrorState_t err_code, Verbosity_t verbose, DoorEvents_t ev) : deb(debup, debdown), block(blck), clearing(clear), txt(err_txt), error_code(err_code), verb(verbose), event(ev){};
